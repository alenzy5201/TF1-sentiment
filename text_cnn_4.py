    ...
    def __init__(
      self, sequence_length, num_classes, embedding_table,
            ...
            with tf.name_scope("embedding_loss"):
                epsilon = 1e-5
                distance = tf.sqrt(tf.reduce_sum((self.y[0] - self.y[1]) ** 2) + epsilon) - tf.sqrt(
                    tf.reduce_sum((self.y[0] - self.y[2]) ** 2) + epsilon) + 5.0
                # distance = (tf.reduce_sum((self.y[0] - self.y[1]) ** 2) + epsilon) - (
                #             tf.reduce_sum((self.y[0] - self.y[2]) ** 2) + epsilon) + 5.0
                # distance =  tf.sqrt( tf.reduce_sum( (self.y[0] - self.y[1])** 2  ) + epsilon)
                # self.embedding_loss = tf.maximum(distance, 0)
                self.embedding_loss = tf.maximum(distance, 0)
