...
TRAIN_EMBEDDING_EPOCH = 0

TRAIN_CLASSIFY_EPOCH = 200000
# Parameters
# ==================================================
...
tf.flags.DEFINE_string("file_dir", ".\Large_Scale_Sentiment_Classification_Data_\Weakly_labeled_data_1.1M", "embedding_train dataSet")
...
def preprocess():
    ...
    max_document_length = data_helper.max_sentence_length([classify_sentences])
    print('句子的最大长度为：{}'.format(max_document_length))

    #Build vocab
    vocab_processor = learn.preprocessing.VocabularyProcessor(max_document_length)
    x = np.array(list(vocab_processor.fit_transform(classify_sentences)))
...
def train(x, y, vocab_processor):
    ...
    with tf.Graph().as_default():
        ...
        test = 10
        if test:
            W = tf.random_normal([len(list(vocab_processor.vocabulary_._mapping)), FLAGS.embedding_dim], stddev=17,mean=23)
        else:
        ...
        with sess.as_default():
            ...
            classify_optimizer = tf.train.AdamOptimizer(1e-3)#-6可以
            #classify_optimizer = tf.train.GradientDescentOptimizer(1e-1)
            ...
            #Tune
            VAL_NUM = 64
            train_classify_epoch = TRAIN_CLASSIFY_EPOCH
            x_train = x[VAL_NUM:]
            y_train = y[VAL_NUM:]
            x_test = x[:VAL_NUM]
            y_test = y[:VAL_NUM]
